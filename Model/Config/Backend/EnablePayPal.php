...
class EnablePayPal extends Enable
{
    protected const PAYMENT_METHOD_CODE = Monei::PAYPAL_CODE;
     ...
     * @return EnablePayPal
     */
    public function beforeSave(): EnablePayPal
    {
        if ($this->getValue() && !$this->isPaymentAvailable()) {
            $this->setValue('0');
            // phpcs:disable
            $this->messageManager->addNoticeMessage(
                __('PayPal payment method is not available in your Monei account. Please enable it in your Monei account to use it.')
            );
            // phpcs:enable
        }
